...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="generation of particle jets via trained EPiC-GAN for JetNet30 / JetNet150 (gluon / light quarks / top jets)"
    )
    ...
    parser.add_argument(
        ...
        "-n",
        default=150,
        help="number of points to generate (either 30 or 150) for JetNet30 or JetNet150",
        type=int,
    )
    parser.add_argument(
        "--dataset_type",
        default="jetnet_top",
        help="define the dataset type, either jetnet_gluon, jetnet_quark, or jetnet_top",
        type=str,
    )
    parser.add_argument(
        "--events",
        ...
        default=100_000,
        type=int,
        help="number of events / number of jets to generate, should be at least 100k to ensure accurate particle multiplicity diversity",
    )
    parser.add_argument("--batch_size", "-b", default=512, type=int, help="batch size")
    parser.add_argument(
        "--output_folder",
        default="/beegfs/desy/user/ewencedr/epic_gan/jets/",
        type=str,
        help="set output folder for generated jets in npy format",
    )
    parser.add_argument(
        "--outfile_name",
        default="EPiC_jets.npy",
        type=str,
        help="output filename for the .npy file containing the generated jets",
    )
    parser.add_argument(
        "--model_name_to_load",
        default="LSGAN_EPiC_GAN_el6_l10_24593_best_model.tar",
        type=str,
    ...
    G = utils.get_model(
        model_name=params.model_name, args=args, only_generator=True
    )  # load generator G
    G = load_model(G, params.model_name_to_load, params)
    ...
    gen_ary = generation(
        ...
        total_gen=params.events,
        n_points_max=params.n_points,
        min_pt=mins[0],
    ...
    print(
        "time per jet: {} microseconds".format(
            (stop_time - start_time) / params.events * 1e6
        )
    )
...
def generation(
    ...
    kde_sample = kde.resample(
        total_gen + 1000
    ).T  # add 1000 events to compensate for removal of points outside the kde window
    ...
    for i in range(len(unique_pts)):
        ...
        while countdown > 0:
            ...
            z_local = utils.get_local_noise(
                current_batch_size, n_points, latent_local, device=device
            )
            ...
            if normalize_points == True:
                gen_tensor = utils.inverse_normalize_tensor(
                    gen_tensor, mean=norm_means, std=norm_stds, sigma=norm_sigma
                )
            else:
            ...
            gen_tensor = np.zeros(
                (gen_tensor_noPad.shape[0], n_points_max, 3)
            )  ## WATCH OUT: here the feature dimension is hardcoded
            gen_tensor[
                0 : gen_tensor_noPad.shape[0],
                0 : gen_tensor_noPad.shape[1],
                0 : gen_tensor_noPad.shape[2],
            ] = gen_tensor_noPad
