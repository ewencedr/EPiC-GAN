...
def order_ary(
    ary, order_dim=0
):  # assumse array to be in shape [events, points, features]
    out_list = []
    events, pints, dims = ary.shape
    sortmask = np.argsort(ary[:, :, order_dim], axis=-1)
    for i in range(dims):
        new = np.take_along_axis(ary[:, :, i], sortmask, axis=-1)[
            :, ::-1
        ]  # order reversed
        out_list.append(new)
...
def order_ary_2(ary1, ary2, order_dim=0):
    ...
    for i in range(dims):
        new1 = np.take_along_axis(ary1[:, :, i], sortmask, axis=-1)[
            :, ::-1
        ]  # order reversed
...
def order_tensor(
    tensor, order_dim=0
):  # assumse array to be in shape [events, points, feats]
    events, points, dims = tensor.size()
    sortmask = torch.argsort(
        tensor[:, :, order_dim].reshape(events, points, 1), dim=1, descending=True
    ).expand(-1, -1, dims)
...
def save_model(G, optimizer_G, C, optimizer_C, fname="tmp", folder="./tmp_trainings/"):
...
def validation_mean_ms(
    ...
    if return_latent_space:
        data_ary, gen_ary, _ = evaluation_loop(
            D,
            f_test,
            f_kde,
            latent,
            ...
            set_min_pt=set_min_pt,
            min_pt=min_pt,
            return_latent_space=return_latent_space,
            gen_size_same_as_data=False,
            center_gen=center_gen,
        )
    else:
        data_ary, gen_ary = evaluation_loop(
            D,
            f_test,
            f_kde,
            latent,
...
def validation_FPND(
    ...
    if return_latent_space:
        _, gen_ary, _ = evaluation_loop(
            ...
            min_pt=min_pt,
            return_latent_space=return_latent_space,
            gen_size_same_as_data=True,
            center_gen=center_gen,
        )
    else:
        _, gen_ary = evaluation_loop(
            D,
            f_test,
            f_kde,
            latent,
...
def evaluation_loop(
    ...
    sampled_points = sampled_points[
        (sampled_points >= 1) & (sampled_points <= n_points_max)
    ]
    ...
    for i in range(len(unique_pts)):
        ...
        while countdown > 0:
            ...
            z_local = get_local_noise(current_batch_size, n_points, latent_local).to(
                device
            )
            z_global = get_global_noise(current_batch_size, latent).to(device)
            if return_latent_space:
                reco_test, latent_tensor = D(z_global, z_local)
                reco_test, latent_tensor = (
                    reco_test.detach().cpu().numpy(),
                    latent_tensor.detach().cpu().numpy(),
                )
            ...
            if normalize_points == True:
                reco_test = inverse_normalize_tensor(
                    torch.tensor(reco_test),
                    mean=norm_means,
                    std=norm_stds,
                    sigma=norm_sigma,
                ).numpy()
            ...
            reco_test = np.zeros(
                (reco_test_noPad.shape[0], n_points_max, 3)
            )  ## WATCH OUT: here the feature dimension is hardcoded
            reco_test[
                0 : reco_test_noPad.shape[0],
                0 : reco_test_noPad.shape[1],
                0 : reco_test_noPad.shape[2],
            ] = reco_test_noPad
    ...
    if calibrate_pt:
        ...
        if set_min_pt:
            gen_ary[..., 0][
                (gen_ary[..., 0] < min_pt) & (gen_ary[..., 0] != 0.0)
            ] = min_pt
...
def plot_overview(
    ...
    x_min, x_max = (
        np.array(
            [
                data_test_ary[:, :, 0].flatten().min(),
                reco_test_ary[:, :, 0].flatten().min(),
            ]
        ).min(),
        np.array(
            [
                data_test_ary[:, :, 0].flatten().max(),
                reco_test_ary[:, :, 0].flatten().max(),
            ]
        ).max(),
    )
    x_min, x_max = 0, 1.0
    hist1 = ax.hist(
        data_test_ary[:, :, 0][data_test_ary[:, :, 0] != 0].flatten(),
        bins=100,
        label=dataset_name,
        histtype="stepfilled",
        range=[x_min, x_max],
    ...
    hist2 = ax.hist(
        reco_test_ary[:, :, 0][reco_test_ary[:, :, 0] != 0].flatten(),
        bins=100,
    ...
    x_min, x_max = (
        np.array(
            [
                data_test_ary[:, :, 1].flatten().min(),
                reco_test_ary[:, :, 1].flatten().min(),
            ]
        ).min(),
        np.array(
            [
                data_test_ary[:, :, 1].flatten().max(),
                reco_test_ary[:, :, 1].flatten().max(),
            ]
        ).max(),
    )
    x_min, x_max = -1.6, 1.2
    hist1 = ax.hist(
        data_test_ary[:, :, 1][data_test_ary[:, :, 1] != 0].flatten(),
        bins=100,
        label=dataset_name,
        histtype="stepfilled",
        range=[x_min, x_max],
    ...
    hist2 = ax.hist(
        reco_test_ary[:, :, 1][reco_test_ary[:, :, 1] != 0].flatten(),
        bins=hist1[1],
        label=generator_name,
        histtype="step",
        range=[x_min, x_max],
    ...
    x_min, x_max = (
        np.array(
            [
                data_test_ary[:, :, 2].flatten().min(),
                reco_test_ary[:, :, 2].flatten().min(),
            ]
        ).min(),
        np.array(
            [
                data_test_ary[:, :, 2].flatten().max(),
                reco_test_ary[:, :, 2].flatten().max(),
            ]
        ).max(),
    )
    x_min, x_max = -0.5, 0.5
    hist1 = ax.hist(
        data_test_ary[:, :, 2][data_test_ary[:, :, 2] != 0].flatten(),
        bins=100,
        label=dataset_name,
        histtype="stepfilled",
        range=[x_min, x_max],
    ...
    hist2 = ax.hist(
        reco_test_ary[:, :, 2][reco_test_ary[:, :, 2] != 0].flatten(),
        bins=hist1[1],
        label=generator_name,
        histtype="step",
        range=[x_min, x_max],
    ...
    for j in range(3):
        ...
        x_min, x_max = (
            np.array(
                [
                    data_test_ary[:, i, 0].flatten().min(),
                    reco_test_ary[:, i, 0].flatten().min(),
                ]
            ).min(),
            np.array(
                [
                    data_test_ary[:, i, 0].flatten().max(),
                    reco_test_ary[:, i, 0].flatten().max(),
                ]
            ).max(),
        )
        hist1 = ax.hist(
            data_test_ary[:, i, 0][data_test_ary_p4s[:, i, 0] != 0].flatten(),
            bins=100,
            label=dataset_name,
            histtype="stepfilled",
            alpha=0.5,
        ...
        hist2 = ax.hist(
            reco_test_ary[:, i, 0][reco_test_ary_p4s[:, i, 0] != 0].flatten(),
            bins=100,
            label=generator_name,
            histtype="step",
            density=density,
        ...
        if i == 0:
            ax.set_xlabel(
                "{}".format(i + 1)
                + r"$^\mathrm{st}$ relative particle $p_\mathrm{T}^\mathrm{rel}$",
                fontsize=24,
            )
        elif i == 1:
            ax.set_xlabel(
                "{}".format(i + 1)
                + r"$^\mathrm{nd}$ relative particle $p_\mathrm{T}^\mathrm{rel}$",
                fontsize=24,
            )
        elif i == 2:
            ax.set_xlabel(
                "{}".format(i + 1)
                + r"$^\mathrm{rd}$ relative particle $p_\mathrm{T}^\mathrm{rel}$",
                fontsize=24,
            )
        else:
            ax.set_xlabel(
                "{}".format(i + 1)
                + r"$^\mathrm{th}$ relative particle $p_\mathrm{T}^\mathrm{rel}$",
                fontsize=24,
            )
        ax.set_ylabel("particles", fontsize=24)
        ax.set_yscale("log")
        ax.set_xticks(
            ax.get_xticks()[1:][::2]
        )  # hide every second x_tick, starting from the second
        ax.tick_params(labelsize=20)

    # jet mults
    ax = fig.add_subplot(gs[6])
    x_min, x_max = (
        np.array([data_mults.min(), reco_mults.min()]).min(),
        np.array([data_mults.max(), reco_mults.max()]).max(),
    )
    b = x_max - x_min + 1
    hist1 = ax.hist(
        data_mults,
    ...
    x_min, x_max = (
        np.array([data_ms.min(), reco_ms.min()]).min(),
        np.array([data_ms.max(), reco_ms.max()]).max(),
    )
    x_min, x_max = 0, 0.3
    hist1 = ax.hist(
        data_ms,
    ...
    x_min, x_max = (
        np.array([data_pts.min(), reco_pts.min()]).min(),
        np.array([data_pts.max(), reco_pts.max()]).max(),
    )
    x_min, x_max = 0.6, 1.2
    hist1 = ax.hist(
        data_pts,
...
def get_model(model_name, args, latent=10, only_generator=False):
    ...
    if model_name == "EPiC_GAN":
        if not only_generator:
            C = models.EPiC_discriminator(args).to(
                device
        ...
        if latent == 0:
            ...
            G = models.DeepSet_generator(args).to(device)  # generator
    else:
